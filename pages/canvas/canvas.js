...
Page({
  ...
  createNewImg: function() {
    ...
    that.drawInnerShape(ctx,
      Math.round(that.data.windowWidth * 0.6),
      Math.round(that.data.windowWidth * 0.4),
      Math.round(that.data.windowWidth * 0.82),
      5,
      Math.round(that.data.windowWidth * 0.6 / (maxRad + 2)));
    ...
    that.data.powerLevel = parseInt(app.globalData.patience) + parseInt(app.globalData.kindness) + parseInt(app.globalData.agility) + parseInt(app.globalData.personality) + parseInt(app.globalData.responsibility)
    ...
    if ((maxRad - parseInt(app.globalData.patience)) > 0) {
      patience = maxRad - parseInt(app.globalData.patience)
    }
    if ((maxRad - parseInt(app.globalData.kindness)) > 0) {
      kind = maxRad - parseInt(app.globalData.kindness)
    }
    if ((maxRad - parseInt(app.globalData.agility)) > 0) {
      agility = maxRad - parseInt(app.globalData.agility)
    }
    if ((maxRad - parseInt(app.globalData.personality)) > 0) {
      personality = maxRad - parseInt(app.globalData.personality)
    }
    if ((maxRad - parseInt(app.globalData.responsibility)) > 0) {
      responsibility = maxRad - parseInt(app.globalData.responsibility)
    }

    var levelArray = new Array(patience, kind, agility, personality, responsibility);
  ...
  drawFont: function(ctx, content) {
    ...
    ctx.setFontSize(parseInt(this.data.windowWidth * 0.07));
  ...
  drawFont2: function(ctx, content) {
    ctx.setGlobalAlpha(1);
    ctx.setFontSize(parseInt(this.data.windowWidth * 0.2));
    ctx.setFillStyle("#ff0000");
    ctx.fillText(content, this.data.windowWidth * 0.45, this.data.windowWidth * 0.38);
  },
